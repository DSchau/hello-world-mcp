...
function combinedAuthMiddleware(req: Request, res: Response, next: () => void) {
  const authHeader = req.headers.authorization;

  const expectedUsername = "admin";
  const expectedPassword = "password";
  const expectedBearerToken = Buffer.from(`${expectedUsername}:${expectedPassword}`).toString("base64");

  if (!authHeader) {
    res.setHeader("WWW-Authenticate", 'Basic realm="Postman Echo", Bearer');
...
app.all('/', combinedAuthMiddleware as any, async (req: Request, res: Response) => {
